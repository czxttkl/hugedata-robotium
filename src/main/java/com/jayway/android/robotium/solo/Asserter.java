...
class Asserter {
	...
	{
		Assert.assertTrue(message, waiter.waitForActivity(name, timeout));
	}
	...
	/**
	 * Asserts that the specified view is shown or visible.
	 */
	...
	/**
	 * Asserts that the specified view is shown or visible and take screenshots.
	 */

	public void assertViewShown(String message, View view, int timeout, Solo solo)
	{
		if(!waiter.isViewShown(view, timeout)){
			solo.takeScreenshot("test_" + message + "_Failure");
		    Assert.assertTrue(message, false);
		}else {
			solo.takeScreenshot("test_" + message + "_Success");
